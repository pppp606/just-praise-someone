...
describe("PraiseService", () => {
  describe("createPraise", () => {
    ...
    test("skillCodesがMAX_SKILL_COUNTを超える場合、ValidationErrorを返す", async () => {
      const data = {
        content: "a".repeat(MIN_CONTENT_LENGTH),
        receivedUserId: "test_user_1",
        skillCodes: ["code_quality", "debugging", "code_review", "security", "hacking"],
        givenUserId: "test_user_2",
      };
    ...
    test("同じユーザーに対する褒め言葉が既に存在する場合、DuplicateErrorを返す", async () => {
      ...
      const skill = await prisma.skill.findFirst({ where: { code: "code_quality" } });
      if (!skill) {
  ...
  describe("deletePraise", () => {
    test("存在しない褒め言葉を削除しようとした場合、NotFoundErrorを返す", async () => {
      ...
      await expect(PraiseService.deletePraise(praiseId, "test_user_1")).rejects.toEqual({
        code: "notFound",
      });
    });
    ...
    test("投稿者以外が褒め言葉を削除しようとした場合、UnauthorizedErrorを返す", async () => {
      ...
      await expect(PraiseService.deletePraise(createdPraise.id, "test_user_1")).rejects.toEqual({
        code: "unauthorized",
      });

      const praise = await prisma.praise.findUnique({ where: { id: createdPraise.id } });
    ...
    test("正常なデータの場合、褒め言葉を削除する", async () => {
      ...
      const deletedPraise = await prisma.praise.findUnique({ where: { id: createdPraise.id } });
      expect(deletedPraise).toBeNull();

      const deletedSkill = await prisma.praiseSkill.findFirst({ where: { praiseId: createdPraise.id } });
  ...
  describe("updatePraise", () => {
    test("存在しない褒め言葉を更新しようとした場合、NotFoundErrorを返す", async () => {
      const praiseId = "invalid_praise_id";
      await expect(PraiseService.updatePraise(praiseId, "test_user_1", {
        content: "a".repeat(MIN_CONTENT_LENGTH),
        skillCodes: ["code_quality"],
      })).rejects.toEqual({
        code: "notFound",
    ...
    test("投稿者以外が褒め言葉を更新しようとした場合、UnauthorizedErrorを返す", async () => {
      ...
      await expect(PraiseService.updatePraise(createdPraise.id, "test_user_1", {
        content: "a".repeat(MIN_CONTENT_LENGTH),
        skillCodes: ["code_quality"],
      })).rejects.toEqual({
        code: "unauthorized",
    ...
    test("contentがMIN_CONTENT_LENGTH未満の場合、ValidationErrorを返す", async () => {
      ...
      await expect(PraiseService.updatePraise(createdPraise.id, "test_user_2", data)).rejects.toEqual({
        code: "validationError",
        message: `褒め言葉は${MIN_CONTENT_LENGTH}文字以上${MAX_CONTENT_LENGTH}文字以下で記入してください。`,
      });
    });
